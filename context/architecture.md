...
| Background       | Trigger.dev             | Durable AI generation workflows                                |
| Artifact storage | Vercel Blob             | Canvas snapshots and generated Markdown specs                  |

## System Boundaries

- app/api - Authenticated request handlers: input validation, ownership checks,
  task triggering, and persistence.
- trigger - Long-running background jobs: Al design generation and spec
  generation.
- lib - Shared infrastructure: Prisma client, access control helpers, and utilities.
- components - Ul composition: canvas surfaces, sidebars, dialogs, and
...
- Vercel Blob: generated artifacts - canvas snapshots at canvas/{projectId}.json
  and specs at specs/{projectId}/{specId}.md.
- Project records, spec records, and task run records belong in PostgreSQL.
- Canvas content and Markdown output are stored in and retrieved from Vercel
  Blob.
- The blob URL is stored in the database ( canvasJsonPath,
...
· Output: structured node and edge updates written into the shared Liveblocks
room.
...
- Execution: durable background task via Trigger.dev.
- Output: Markdown technical spec saved to the filesystem and linked to the
  project in the database.

## Invariants

1. Request handlers do not run long-lived Al work - that belongs in background
   tasks.
2. Metadata and large generated artifacts are stored in separate layers.
3. Auth and ownership are enforced at every mutation boundary.
4. Client components are used only where browser interactivity or real-time state
   requires them.
